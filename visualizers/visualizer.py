...
from data_sources import metars, weather, daylight
...
color_by_rules = {
    metars.IFR: rgb_colors[colors_lib.RED],
    metars.VFR: rgb_colors[colors_lib.GREEN],
    metars.MVFR: rgb_colors[colors_lib.BLUE],
    metars.LIFR: rgb_colors[colors_lib.MAGENTA],
    metars.NIGHT: rgb_colors[colors_lib.YELLOW],
    metars.NIGHT_DARK: rgb_colors[colors_lib.DARK_YELLOW],
    metars.SMOKE: rgb_colors[colors_lib.GRAY],
    metars.INVALID: rgb_colors[colors_lib.OFF],
    metars.INOP: rgb_colors[colors_lib.OFF],
}
...
def __get_night_color_to_render__(color_by_category: list, proportions: list) -> list:
    ...
    # is in a twilight period.
        ...
        if proportions[0] > 0.0 or proportions[1] < 1.0:
            color_to_render = color_by_rules[metars.NIGHT]
        ...
        color_to_render = colors_lib.get_color_mix(
            ...
            color_by_rules[metars.NIGHT],
            proportions[0],
        )
    elif proportions[1] > 0.0:
        color_to_render = colors_lib.get_color_mix(
            color_by_rules[metars.NIGHT], color_by_category, proportions[1]
        )
...
def get_mix_and_color(color_by_category, airport):
    ...
    proportions = daylight.get_twilight_transition(airport)
...
class Visualizer(object):
    ...
    def __get_brightness_adjusted_color__(
        ...
        proportions, color_to_render = get_mix_and_color(starting_color, station)
        brightness_adjustment = configuration.get_brightness_proportion()
